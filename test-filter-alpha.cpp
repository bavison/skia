...
struct SkBitmapProcState : public SkBitmapProcInfo {
    SkBitmapProcState(const void* addr, size_t rowBytes, uint16_t fAlphaScale) : SkBitmapProcInfo(addr, rowBytes), fAlphaScale(fAlphaScale) {}
...
static __attribute__((always_inline)) inline void Filter_32_alpha_neon(unsigned x, unsigned y,
...
void S32_alpha_D32_filter_DX_neon(const SkBitmapProcState& s,
    ...
    do {
        ...
        Filter_32_alpha_neon(subX, subY, row0[x0], row0[x1], row1[x0], row1[x1], colors, alphaScale);
        colors += 1;
    } while (--count != 0);
}
...
void __attribute__((noinline)) new_S32_alpha_D32_filter_DX_neon(const SkBitmapProcState& s,
                          const uint32_t* __restrict__ xy,
                          int count, SkPMColor* __restrict__ colors) {
    unsigned alphaScale = s.fAlphaScale;
    const char* __restrict__ srcAddr = (const char*)s.fPixmap.addr();
...
int main(void)
{
    ...
    for (size_t offset = 0; offset <= 4; offset += 1) {
        for (size_t len = 1; len <= PIXELS - offset; len += 1) {
            S32_alpha_D32_filter_DX_neon(SkBitmapProcState(src, PIXELS * sizeof(SkPMColor), 0x80), xy, len, dst1 + offset);
            new_S32_alpha_D32_filter_DX_neon(SkBitmapProcState(src, PIXELS * sizeof(SkPMColor), 0x80), xy, len, dst2 + offset);
...
//#define CANDIDATE S32_alpha_D32_filter_DX_neon
#define CANDIDATE new_S32_alpha_D32_filter_DX_neon

    uint64_t t1, t2, t3;
    uint32_t byte_cnt;
    static SkPMColor bufa[ROWS+1][PIXELS];
    static SkPMColor bufb[ROWS][PIXELS];
    SkBitmapProcState s(bufa, PIXELS * sizeof(SkPMColor), 0x80);
#define VALUE 0x80
